...
class ClubDashboardController extends Controller
{
        ...
        return response()->json([
            ...
            'data' => [
                'complejo' => [
                    ...
                    'hora_inicio_pico_semana' => $complejo->hora_inicio_pico_semana ? substr($complejo->hora_inicio_pico_semana, 0, 5) : '18:00',
                    'hora_fin_pico_semana' => $complejo->hora_fin_pico_semana ? substr($complejo->hora_fin_pico_semana, 0, 5) : '23:00',
                    'dias_pico_semana' => $complejo->dias_pico_semana ?? [1, 2, 3, 4, 5],
    ...
    public function storeCancha(Request $request, string $subdomain): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'precio_luz_adicional' => 'nullable|numeric|min:0',
            'techada' => 'boolean',
            'iluminacion' => 'boolean',
    ...
    public function updateConfiguracion(Request $request, string $subdomain): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'hora_inicio_pico_semana' => ['nullable', 'string', 'regex:/^([01][0-9]|2[0-3]):[0-5][0-9]$/'],
            'hora_fin_pico_semana' => ['nullable', 'string', 'regex:/^([01][0-9]|2[0-3]):[0-5][0-9]$/'],
            'dias_pico_semana' => ['nullable', 'array'],
            'dias_pico_semana.*' => ['integer', 'between:0,6'],
            'dias_fin_semana' => ['nullable', 'array'],
            'dias_fin_semana.*' => ['integer', 'between:0,6'],

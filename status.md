...
- [x] **Visualización de "Reseña Corta / Sobre Nosotros" en Todas las Plantillas Web, Footer y Mockup:** Renderizado de `descripcion_corta` en la plantilla `ModernShowcaseTemplate` (banner Hero con card glassmorphic) y `BookingDirectTemplate` (subheader contextual bajo el título), unificación con `InstitucionalTemplate`, renderizado dinámico en la columna 1 de `ClubFooter` en home y páginas CMS, y reflejo interactivo en tiempo real en el mockup de teléfono del panel de administración (`BrandingClubPanel`) *(Completado)*
...
### Caso de Prueba: Configuración y Visualización de "Reseña Corta / Sobre Nosotros" en Web Pública y Footer
1. **Acceso al Panel de Marca del Club:**
   - Ingresar a `http://club-padel-bronce.localhost:8080/panel`.
   - Ir a la pestaña **Sitio Web** y luego a la subpestaña **Identidad & Diseño Web**.
2. **Edición de la Reseña Corta:**
   - Ubicar el campo **Reseña Corta / Sobre Nosotros (máx. 1000 caracteres)**.
   - Escribir o modificar el texto descriptivo del club (ej. `"Somos un club nuevo y tenemos de ídolo a Nico. Instalaciones de primer nivel con canchas de cristal y servicio integral de buffet."`).
   - Comprobar que en el recuadro **Previsualización en Vivo** (mockup de teléfono simulado a la derecha), el texto se actualiza en tiempo real debajo del eslogan.
   - Presionar **Guardar Cambios** y comprobar la notificación toast verde de confirmación.
3. **Verificación en el Sitio Web Público:**
   - Ingresar en el navegador a `http://club-padel-bronce.localhost:8080/`.
   - **En el Banner Principal (Hero):**
     * Verificar que en la tarjeta central o lateral sobre la portada se exhibe claramente la reseña corta configurada, con excelente legibilidad y diseño translúcido acorde a los colores del club.
   - **En el Pie de Página (Footer):**
     * Desplazarse al pie de página (Columna 1: Información del Club).
     * Constatar que debajo del nombre del club se renderiza la reseña corta personalizada en lugar del texto genérico.
4. **Verificación en Páginas Institucionales CMS:**
   - Ingresar a cualquier página secundaria del club (ej. `http://club-padel-bronce.localhost:8080/paginas/quienes-somos`).
   - Comprobar que en el pie de página también se visualiza la reseña corta del club.

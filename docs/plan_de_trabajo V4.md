...
## 2. Modelo de Negocio Híbrido: Planes Escalonados y Sistema Modular (Feature Flags / Add-ons)

Para maximizar la conversión comercial y permitir que cada complejo adquiera solo las herramientas que necesita, la plataforma adopta un modelo escalonado por niveles (Bronce, Plata, Oro) complementado con un sistema de **Módulos / Add-ons activables a la carta** por el Super Administrador.

### 2.1. Estructura de Planes Base

| Característica / Módulo | Plan Bronce (Operativo Esencial) | Plan Plata (Gestión Club ERP) | Plan Oro (Club Digital Full) |
| :--- | :---: | :---: | :---: |
| **Buscador & Directorio Global** | ✅ | ✅ | ✅ |
| **Grilla Horaria & Motor de Reservas** | ✅ | ✅ | ✅ |
| **Cobro Online de Señas / Pagos** | ✅ | ✅ | ✅ |
| **Gestión de Turnos Fijos (Recurrentes)** | ❌ | ✅ | ✅ |
| **Punto de Venta (POS / Buffet) y Stock** | ❌ | ✅ | ✅ |
| **Control de Caja y Reportes Diarios** | ❌ | ✅ | ✅ |
| **Partidos Abiertos & Split Payment** | ❌ | ❌ | ✅ |
| **Torneos, Ligas y Escuelitas** | ❌ | ❌ | ✅ |
| **Generador de Sitio Web (CMS + Subdominio)**| ❌ | ❌ | ✅ |
| **Integración Domótica IoT (Luces/Accesos)** | ❌ | ❌ | ✅ |

### 2.2. Sistema Modular y Add-ons a la Carta
Cualquier complejo suscripto a un plan inferior (ej. Bronce o Plata) puede contratar o recibir la activación individual de módulos específicos sin necesidad de saltar forzadamente al siguiente plan completo:
*   **Add-on Torneos & Ligas:** Activación del gestor de fixtures y tablas de posiciones.
*   **Add-on Sitio Web Propio (CMS Multitenant):** Habilitación de subdominio independiente, plantillas y editor de páginas institucionales.
*   **Add-on Punto de Venta / Buffet:** Habilitación de stock y comanda vinculada a turnos.
*   **Add-on Domótica IoT:** Módulo de comunicación con hardware para encendido automático de luces.
*   **Modo Trial / Pruebas Temporales:** El Super Administrador puede habilitar cualquier módulo con fecha de vencimiento (ej. 15 días gratis) para acciones comerciales y fidelización.

### 2.3. Arquitectura Técnica de Entitlements (Feature Flags)

#### Esquema de Base de Datos para Módulos Dinámicos
*   `modulos`: `id`, `nombre`, `slug` (`reservas`, `pos_buffet`, `torneos`, `cms_web`, `domotica`, `split_payment`, `turnos_fijos`), `descripcion`.
*   `planes`: `id`, `nombre` (`Bronce`, `Plata`, `Oro`), `precio_mensual`, `estado`.
*   `plan_modulo` (pivote): `plan_id`, `modulo_id`. Define los módulos incluidos por defecto en cada plan.
*   `complejo_modulo` (pivote con control granular): `complejo_id`, `modulo_id`, `esta_activo` (boolean), `valido_hasta` (timestamp nullable, para pruebas temporales).

#### Control en Backend (Laravel Middleware & Policies)
La API protege los endpoints mediante un Middleware dedicado que evalúa en tiempo de ejecución si el complejo tiene contratado el plan correspondiente o el add-on activo:

```php
Route::middleware(['auth:sanctum', 'tenant.has_module:torneos'])->group(function () {
    Route::post('/torneos', [TorneoController::class, 'store']);
    Route::get('/torneos/{id}/fixture', [TorneoController::class, 'fixture']);
});
```
*   *Respuesta ante denegación:* Si el complejo no tiene el módulo contratado, el sistema devuelve `403 Forbidden` con payload estructurado `{ "error": "MODULE_NOT_ENABLED", "module": "torneos", "upgrade_url": "/admin/suscripcion" }`.

#### Control en Frontend (Next.js Panel Admin)
*   **Panel del Super Administrador:** Pantalla con conmutadores (*toggles*) por complejo para activar/desactivar módulos en tiempo real o asignar períodos de prueba.
*   **Panel del Club:** El menú lateral de navegación consulta los módulos activos (`useTenantModules()`) y oculta o muestra con ícono de bloqueo (*candado*) las secciones no contratadas, guiando al usuario hacia el flujo de contacto o actualización de suscripción.

### 2.4. Roles y Permisos (RBAC)
*   **Super Administrador:** Control de métricas SaaS, gestión de planes, cobro de suscripciones y activación modular por cliente.
*   **Administrador / Dueño de Complejo:** Reportes financieros, configuración de canchas, tarifas, empleados y módulos contratados.
*   **Operador de Mostrador / Empleado:** Control diario de agenda, cobro en mostrador, adición de consumos al turno y caja.
*   **Profesor / Organizador de Torneo:** Creación de fixtures, resultados y padrón de alumnos.
*   **Cliente (Jugador):** Reservas, pago dividido (*split payment*), inscripción a torneos y búsqueda de partidos.
...
### Módulo F: Creador de Sitios Web Dinámicos (CMS Multitenant)
...
Semanas 06-07: [Módulo POS / Buffet, Control de Caja, Reportes y Engine de Feature Flags]
...
### Detalle de Fases
*   **Fase 1 (Semanas 1-2): Infraestructura & Tenancy Base:** Entorno Docker Compose, repositorio GitHub con workflows CI/CD, configuración de Caddy con validación de dominios y esquema relacional multitenant con soporte de módulos.
...
*   **Fase 3 (Semanas 6-7): Gestión de Club (ERP) & Feature Flags:** Inventario de mostrador, comandas asociadas a turnos, módulos de apertura/cierre de caja, filtros de sanitización XSS y middleware de control modular.
